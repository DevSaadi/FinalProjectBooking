...
export class BusComponent implements OnInit {
  title = "Bus";
  role
  public baseUrl: string = 'https://localhost:44342';
  public listBus: any = [];
  public res: any;
  ...
  public isGenerateData: boolean = false;
  exportToExcel() {
    this.isGenerateData = true;
    var d = new Date();
    var sec = d.getTime() / 1000;
    var strTime = sec.toString();
    this.reportDate = strTime;
    setTimeout(() => {
      var blob = new Blob([document.getElementById('listBus').innerHTML], {
        type: 'application/vnd.ms-excel;charset=charset=utf-8'
      });
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.download = 'bus' + this.reportDate + '.xls';
      a.href = url;
      a.textContent = 'bus' + this.reportDate + '.xls';
      a.click();
  ...
  exportToPDF() {
    ...
    setTimeout(() => {
      ...
      var url = URL.createObjectURL(blob);
      var a = document.createElement('a');
      a.download = 'bus' + this.reportDate + '.pdf';
      a.href = url;
      a.textContent = 'bus' + this.reportDate + '.pdf';
      a.click();
      this.isGenerateData = false;
    }, 0);
  }
